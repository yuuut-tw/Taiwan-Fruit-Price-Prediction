...
#     sc = StandardScaler()
    sc = RobustScaler(copy=True, quantile_range=(25.0, 75.0),
...
def split_Xy(input_set_sc, n_past, n_future):
    ...
    for i in range(n_past, len(input_set_sc)-n_future+1):                               # 扣掉最後預測天數以符合y_test的length 
        X.append(input_set_sc[i-n_past:i, :])                                           # X = 前30天資料
        y.append(input_set_sc[i:i+n_future, 0])                                         # y = 後7天價格
...
def model_creator(n_past, n_features, output):
    ...
    model.add(Dense(output, activation="selu"))    
